...
class ProcessValuePropositionRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        return [
            ...
            'product' => ['required', 'string', 'max:128', new ValidateBadWordsRule()],
            'audience' => ['required', 'string', 'max:256', new ValidateBadWordsRule()],
            'description' => ['required', 'string', 'max:1024', new ValidateBadWordsRule()],
            'tone' => ['required', 'in:' . implode(',', array_keys(config('completions.tones'))), new ValidateBadWordsRule()],
