@section('site_title', formatTitle([__('Feature section'), __('Template'), config('settings.title')]))
...
<div class="d-flex">
    <h1 class="h2 mb-0 text-break">{{ __('Feature section') }}</h1>
</div>
    ...
    <div class="col-12 col-lg-5 px-2">
        <div class="card border-0 shadow-sm mt-3 @if(isset($documents)) d-none d-lg-flex @endif" id="ai-form">
            ...
            <div class="card-body position-relative">
                ...
                <form action="{{ route('templates.feature_section') }}" method="post" enctype="multipart/form-data" @cannot('templates', ['App\Models\User']) class="position-relative opacity-20" @endcannot>
                    ...
                    <div class="form-group">
                        ...
                        @endif
                        <small class="form-text text-muted">{{ __('The title of the feature.') }}</small>
                    </div>

                    <div class="form-group">
                        <label for="i-product">{{ __('Product') }}</label>
                        <input type="text" name="product" id="i-product" class="form-control{{ $errors->has('product') ? ' is-invalid' : '' }}" value="{{ $product ?? (old('product') ?? '') }}" placeholder="{{ __('YouTube, Netflix, Disney') }}">
                        @if ($errors->has('product'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('product') }}</strong>
                            </span>
                        @endif
                        <small class="form-text text-muted">{{ __('The name of the product or service.') }}</small>
                    </div>

                    <div class="form-group">
                        <label for="i-audience">{{ __('Audience') }}</label>
                        <input type="text" name="audience" id="i-audience" class="form-control{{ $errors->has('audience') ? ' is-invalid' : '' }}" value="{{ $audience ?? (old('audience') ?? '') }}" placeholder="{{ __('Musicians, filmmakers, gamers') }}">
                        @if ($errors->has('audience'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('audience') }}</strong>
                            </span>
                        @endif
                        <small class="form-text text-muted">{{ __('The audience of the product or service.') }}</small>
                    </div>

                    <div class="form-group">
                        <label for="i-description">{{ __('Description') }}</label>
                        <textarea rows="3" data-auto-resize-textarea="true" name="description" id="i-description" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" placeholder="{{ __('Creating content that inspires and entertains') }}">{{ $description ?? (old('description') ?? '') }}</textarea>
                        @if ($errors->has('description'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('description') }}</strong>
                            </span>
                        @endif
                        <small class="form-text text-muted">{{ __('The description of the product or service.') }}</small>
                    </div>
                    ...
                    <div class="row mx-n2">
                        ...
                        <div class="col-auto px-2">
                            <a href="{{ route('templates.feature_section') }}" class="btn btn-outline-secondary d-none {{ (__('lang_dir') == 'rtl' ? 'mr-auto' : 'ml-auto') }}">{{ __('Reset') }}</a>
