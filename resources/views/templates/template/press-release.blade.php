@section('site_title', formatTitle([__('Press release'), __('Template'), config('settings.title')]))
...
<div class="d-flex">
    <h1 class="h2 mb-0 text-break">{{ __('Press release') }}</h1>
</div>
    ...
    <div class="col-12 col-lg-5 px-2">
        <div class="card border-0 shadow-sm mt-3 @if(isset($documents)) d-none d-lg-flex @endif" id="ai-form">
            ...
            <div class="card-body position-relative">
                ...
                <form action="{{ route('templates.press_release') }}" method="post" enctype="multipart/form-data" @cannot('templates', ['App\Models\User']) class="position-relative opacity-20" @endcannot>
                    ...
                    <div class="form-group">
                        <label for="i-company">{{ __('Company') }}</label>
                        <input type="text" name="company" id="i-company" class="form-control{{ $errors->has('company') ? ' is-invalid' : '' }}" placeholder="{{ __('Youtube, Netflix, Disney') }}" value="{{ $company ?? (old('company') ?? '') }}">
                        @if ($errors->has('company'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('company') }}</strong>
                            </span>
                        @endif
                        <small class="form-text text-muted">{{ __('The name of the company.') }}</small>
                    </div>

                    <div class="form-group">
                        <label for="i-description">{{ __('Description') }}</label>
                        <textarea rows="3" data-auto-resize-textarea="true" name="description" id="i-description" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" placeholder="{{ __('We strive to be the best when it comes to content creation') }}">{{ $description ?? (old('description') ?? '') }}</textarea>
                        @if ($errors->has('description'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('description') }}</strong>
                            </span>
                        @endif
                        <small class="form-text text-muted">{{ __('The description of the company.') }}</small>
                    </div>

                    <div class="form-group">
                        <label for="i-description">{{ __('Subject') }}</label>
                        <textarea rows="3" data-auto-resize-textarea="true" name="subject" id="i-subject" class="form-control{{ $errors->has('subject') ? ' is-invalid' : '' }}" placeholder="{{ __('The release of our mobile app') }}">{{ $subject ?? (old('subject') ?? '') }}</textarea>
                        @if ($errors->has('subject'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('subject') }}</strong>
                            </span>
                        @endif
                        <small class="form-text text-muted">{{ __('The subject of the press release.') }}</small>
                    </div>
                    ...
                    <div class="row mx-n2">
                        ...
                        <div class="col-auto px-2">
                            <a href="{{ route('templates.press_release') }}" class="btn btn-outline-secondary d-none {{ (__('lang_dir') == 'rtl' ? 'mr-auto' : 'ml-auto') }}">{{ __('Reset') }}</a>
