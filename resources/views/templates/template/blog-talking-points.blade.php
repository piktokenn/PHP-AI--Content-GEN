@section('site_title', formatTitle([__('Blog talking points'), __('Template'), config('settings.title')]))
...
<div class="d-flex">
    <h1 class="h2 mb-0 text-break">{{ __('Blog talking points') }}</h1>
</div>
    ...
    <div class="col-12 col-lg-5 px-2">
        <div class="card border-0 shadow-sm mt-3 @if(isset($documents)) d-none d-lg-flex @endif" id="ai-form">
            ...
            <div class="card-body position-relative">
                ...
                <form action="{{ route('templates.blog_talking_points') }}" method="post" enctype="multipart/form-data" @cannot('templates', ['App\Models\User']) class="position-relative opacity-20" @endcannot>
                    ...
                    <div class="form-group">
                        ...
                        @endif
                        <small class="form-text text-muted">{{ __('The title of the blog post.') }}</small>
                    </div>

                    <div class="form-group">
                        <label for="i-subheading">{{ __('Subheading') }}</label>
                        <input type="text" name="subheading" id="i-subheading" class="form-control{{ $errors->has('subheading') ? ' is-invalid' : '' }}" value="{{ $subheading ?? (old('subheading') ?? '') }}" placeholder="{{ __('Florida, Los Angeles, San Francisco') }}">
                        @if ($errors->has('subheading'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('subheading') }}</strong>
                            </span>
                        @endif
                        <small class="form-text text-muted">{{ __('The subheading of the blog post.') }}</small>
                    </div>
                    ...
                    <div class="row mx-n2">
                        ...
                        <div class="col-auto px-2">
                            <a href="{{ route('templates.blog_talking_points') }}" class="btn btn-outline-secondary d-none {{ (__('lang_dir') == 'rtl' ? 'mr-auto' : 'ml-auto') }}">{{ __('Reset') }}</a>
