@section('site_title', formatTitle([__('Startup ideas'), __('Template'), config('settings.title')]))
...
<div class="d-flex">
    <h1 class="h2 mb-0 text-break">{{ __('Startup ideas') }}</h1>
</div>
    ...
    <div class="col-12 col-lg-5 px-2">
        <div class="card border-0 shadow-sm mt-3 @if(isset($documents)) d-none d-lg-flex @endif" id="ai-form">
            ...
            <div class="card-body position-relative">
                ...
                <form action="{{ route('templates.startup_ideas') }}" method="post" enctype="multipart/form-data" @cannot('templates', ['App\Models\User']) class="position-relative opacity-20" @endcannot>
                    ...
                    <div class="form-group">
                        <label for="i-domains">{{ __('Domains') }}</label>
                        <textarea rows="3" data-auto-resize-textarea="true" name="domains" id="i-domains" class="form-control{{ $errors->has('domains') ? ' is-invalid' : '' }}" placeholder="{{ __('Web programming, digital art, artificial intelligence') }}">{{ $domains ?? (old('domains') ?? '') }}</textarea>
                        @if ($errors->has('domains'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('domains') }}</strong>
                            </span>
                        @endif
                        <small class="form-text text-muted">{{ __('The domains of the startup.') }}</small>
                    </div>
                    ...
                    <div class="row mx-n2">
                        ...
                        <div class="col-auto px-2">
                            <a href="{{ route('templates.startup_ideas') }}" class="btn btn-outline-secondary d-none {{ (__('lang_dir') == 'rtl' ? 'mr-auto' : 'ml-auto') }}">{{ __('Reset') }}</a>
