@section('site_title', formatTitle([__('Freestyle'), __('Template'), config('settings.title')]))
...
<div class="d-flex">
    <h1 class="h2 mb-0 text-break">{{ __('Freestyle') }}</h1>
</div>
    ...
    <div class="col-12 col-lg-5 px-2">
        <div class="card border-0 shadow-sm mt-3 @if(isset($documents)) d-none d-lg-flex @endif" id="ai-form">
            ...
            <div class="card-body position-relative">
                ...
                <form action="{{ route('templates.freestyle') }}" method="post" enctype="multipart/form-data" @cannot('templates', ['App\Models\User']) class="position-relative opacity-20" @endcannot>
                    ...
                    <div class="form-group">
                        ...
                        <textarea rows="3" data-auto-resize-textarea="true" name="prompt" id="i-prompt" class="form-control{{ $errors->has('prompt') ? ' is-invalid' : '' }}" placeholder="{{ __('Write a tagline for a coffee shop') }}">{{ $prompt ?? (old('prompt') ?? '') }}</textarea>
