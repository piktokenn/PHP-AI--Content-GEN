@section('site_title', formatTitle([__('Video tags'), __('Template'), config('settings.title')]))
...
<div class="d-flex">
    <h1 class="h2 mb-0 text-break">{{ __('Video tags') }}</h1>
</div>
    ...
    <div class="col-12 col-lg-5 px-2">
        <div class="card border-0 shadow-sm mt-3 @if(isset($documents)) d-none d-lg-flex @endif" id="ai-form">
            ...
            <div class="card-body position-relative">
                ...
                <form action="{{ route('templates.video_tags') }}" method="post" enctype="multipart/form-data" @cannot('templates', ['App\Models\User']) class="position-relative opacity-20" @endcannot>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="i-title" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" value="{{ $title ?? (old('title') ?? '') }}" placeholder="{{ __('5 steps to becoming a programmer') }}">
                        @if ($errors->has('title'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('title') }}</strong>
                            </span>
                        @endif
                        <small class="form-text text-muted">{{ __('The title of the video.') }}</small>
                    </div>
                    ...
                    <div class="row mx-n2">
                        ...
                        <div class="col-auto px-2">
                            <a href="{{ route('templates.video_tags') }}" class="btn btn-outline-secondary d-none {{ (__('lang_dir') == 'rtl' ? 'mr-auto' : 'ml-auto') }}">{{ __('Reset') }}</a>
