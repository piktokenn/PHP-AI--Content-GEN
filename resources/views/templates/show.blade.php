@section('site_title', formatTitle([e($template->name), __('Template'), config('settings.title')]))
...
<div class="d-flex">
    <h1 class="h2 mb-0 text-break">{{ __($template->name) }}</h1>
</div>
    ...
    <div class="col-12 col-lg-5 px-2">
        <div class="card border-0 shadow-sm mt-3 @if(isset($documents)) d-none d-lg-flex @endif" id="ai-form">
            ...
            <div class="card-body position-relative">
                ...
                <form action="{{ route('templates.show', $template->id) }}" method="post" enctype="multipart/form-data" @cannot('customTemplates', ['App\Models\User']) class="position-relative opacity-20" @endcannot>
                    @cannot('customTemplates', ['App\Models\User'])
                    ...
                    <div class="form-group">
                        ...
                        <textarea rows="3" data-auto-resize-textarea="true" name="prompt" id="i-prompt" class="form-control{{ $errors->has('prompt') ? ' is-invalid' : '' }}" placeholder="{{ $template->prompt }}">{{ $prompt ?? (old('prompt') ?? $template->prompt) }}</textarea>
                    ...
                    <div class="row mx-n2">
                        ...
                        <div class="col-auto px-2">
                            <a href="{{ route('templates.show', $template->id) }}" class="btn btn-outline-secondary d-none {{ (__('lang_dir') == 'rtl' ? 'mr-auto' : 'ml-auto') }}">{{ __('Reset') }}</a>
