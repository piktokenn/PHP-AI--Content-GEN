...
@section('site_title', formatTitle([__('Chats'), __('Developers'), config('settings.title')]))
...
@section('content')
    <div class="bg-base-1 flex-fill">
        <div class="container h-100 py-3 my-3">
            ...
            <h1 class="h2 mb-0 d-inline-block">{{ __('Chats') }}</h1>
...
<div class="bg-dark text-light p-3 rounded d-flex align-items-center mb-3" dir="ltr">
    ...
    <pre class="m-0 text-light">{{ route('api.chats.index') }}</pre>
</div>
...
curl --location --request GET '{{ route('api.chats.index') }}' \
--header 'Accept: application/json' \
--header 'Authorization: Bearer <span class="text-success">{api_key}</span>'
</pre>

                @include('developers.chats.list', ['type' => 0])
                </div>
            </div>
...
<div class="bg-dark text-light p-3 rounded d-flex align-items-center mb-3" dir="ltr">
<span class="badge bg-light text-success px-2 py-1 mr-3">GET</span>
<pre class="m-0 text-light">{!! str_replace(':id', '<span class="text-success">{id}</span>', route('api.chats.show', ['id' => ':id'])) !!}</pre>
</div>
...
<pre class="bg-dark text-light p-3 mb-0 rounded text-left" dir="ltr">
curl --location --request GET '{!! str_replace(':id', '<span class="text-success">{id}</span>', route('api.chats.show', ['id' => ':id'])) !!}' \
--header 'Accept: application/json' \
--header 'Authorization: Bearer <span class="text-success">{api_key}</span>'
</pre>
                </div>
...
<pre class="m-0 text-light">{{ route('api.chats.store') }}</pre>
</div>
                    <p class="mb-1">
                        {{ __('Request example') }}:
                    </p>
<pre class="bg-dark text-light p-3 rounded text-left" dir="ltr">
curl --location --request POST '{{ route('api.chats.store') }}' \
...
--data-urlencode 'description=<span class="text-success">{description}</span>'
                    ...
                    @include('developers.chats.store-update', ['type' => 1])
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm mt-3">
                ...
                <div class="card-body">
                    ...
                    <div class="bg-dark text-light p-3 rounded d-flex align-items-center mb-3" dir="ltr">
                        ...
                        <pre class="m-0 text-light">{!! str_replace(':id', '<span class="text-success">{id}</span>', route('api.chats.update', ['id' => ':id'])) !!}</pre>
                    </div>
                    ...
                    <pre class="bg-dark text-light p-3 rounded text-left" dir="ltr">
curl --location --request PUT '{!! str_replace(':id', '<span class="text-success">{id}</span>', route('api.chats.update', ['id' => ':id'])) !!}' \
--header 'Content-Type: application/x-www-form-urlencoded' \
--header 'Authorization: Bearer <span class="text-success">{api_key}</span>'
</pre>

                    @include('developers.chats.store-update', ['type' => 0])
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm mt-3">
                ...
                <div class="card-body">
                    ...
                    <div class="bg-dark text-light p-3 rounded d-flex align-items-center mb-3" dir="ltr">
                        ...
                        <pre class="m-0 text-light">{!! str_replace(':id', '<span class="text-success">{id}</span>', route('api.chats.destroy', ['id' => ':id'])) !!}</pre>
                    </div>
                    ...
                    <pre class="bg-dark text-light p-3 mb-0 rounded text-left" dir="ltr">
curl --location --request DELETE '{!! str_replace(':id', '<span class="text-success">{id}</span>', route('api.chats.destroy', ['id' => ':id'])) !!}' \
--header 'Authorization: Bearer <span class="text-success">{api_key}</span>'
</pre>
                </div>
            </div>
